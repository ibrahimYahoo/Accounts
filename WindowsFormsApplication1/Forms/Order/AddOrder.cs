        ...
        private void mtCreate_Click(object sender, EventArgs e)
        {
            try
            {
                if (lblCost.Text != "" && dtODate.Text != "" && cbItems.SelectedIndex != -1)
                {
                    ...
                    SqlDataAdapter dad = new SqlDataAdapter("Insert into Orders(OrderId,OrderNo,ODate,PId,IId,Qty,Cost,TotalCost) values(@orderId,@orderno, @orderdate,@PId,@itemid,@qty,@cost,@totalcost)", conn);
